...
# model.load_state_dict(torch.load(r'I:\TCIR-SPLT\ckpt\resnet\ckpt_10.316744081749663.pth')['net'])
model.load_state_dict(torch.load(r'I:\TCIR-SPLT\ckpt\resnet\ckpt_26.12098900803549.pth')['net'])
model = model.eval()
# cam = GradCAM(model=model, target_layers=[model.head.norm], reshape_transform=reshape_transform)
# cam = GradCAM(model=model, target_layers=[model.bn1])
...
for idx in range(len(test_set)):
    ...
    visualization = show_cam_on_image(img_color, grayscale_cam)
